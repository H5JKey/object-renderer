...
int main(int argc, char* argv[]) try {
    ...
    if (eglTarget) {
        ...
        Logger::getInstance().log("Writing into output.png", Logger::Level::INFO);
        utils::writeToPng(egl->getBufferData<uint8_t>(egl->getOutputTexture()), egl->getWidth(), egl->getHeight(), 4,
                          "output.png");
        Logger::getInstance().log("Writing into output_raw.png", Logger::Level::INFO);
        utils::writeToPng(egl->getBufferData<float>(egl->getRawTexture()), egl->getWidth(), egl->getHeight(), 4,
                          "output_raw.png");
        Logger::getInstance().log("Writing into output_albedo.png", Logger::Level::INFO);
        utils::writeToPng(egl->getBufferData<float>(egl->getAlbedoMap()), egl->getWidth(), egl->getHeight(), 4,
                          "output_albedo.png");
        Logger::getInstance().log("Writing into output_normal.png", Logger::Level::INFO);
        utils::writeToPng(egl->getBufferData<float>(egl->getNormalMap()), egl->getWidth(), egl->getHeight(), 4,
                          "output_normal.png");
    }
    Logger::getInstance().log("Renderer application stopped succsessfully", Logger::Level::INFO);
    return EXIT_SUCCESS;
